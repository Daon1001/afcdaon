...
# ── 페이지 설정 ──
st.set_page_config(page_title="벤처인증 AI 마스터 컨설턴트", page_icon="🏛️", layout="wide")
...
<div class="page cover-page">
    ...
    <div class="cover-footer">중소기업경영지원단 · VENTURE CERTIFICATION · {datetime.now().strftime('%Y.%m.%d')}</div>
</div>"""
    n = 0
    for sec in sections:
...
def get_sample_sections():
    return """### [1. 신청기술 요약 및 표준 양식]
- 신청기술명: 데이터기반 고강도·경량화 골판지 박스 구조 최적화 설계 기술
V 기존 시장에 [박스 과잉설계 문제]가 있으며, [경험 의존적 설계]로 어려움
V 당사에서 [FEA 기반 구조 최적화]로 해결, [설계 90% 단축]이라는 차이 보유
V 본 기술 목표는 [통합 자동 설계 시스템] 개발

### [2. 개발배경 및 원인분석]
• 국내 포장산업 약 40조원 규모, 경험 의존적 설계 관행
• 친환경 규제 강화로 재료 절감 요구 급증

### [3. 경쟁력 확보방안]
1. 독자적 AI 알고리즘 보유
2. 설계 리드타임 90% 단축
3. 해외 솔루션 대비 1/3 가격""".split('### ')
        ...
        if not valid_logs:
            ...
            if len(df) == 0:
                ...
                if len(recent) > 0:
                    ...
                    pivot_display = pivot_display.sort_index(ascending=False)  # 최신 날짜 위로
...
with col1:
    ...
    with bc2: biz_item = st.text_input("종목", placeholder="예: 골판지 박스 제조")
...
{{"suggestions": [{{"tech_name": "구체적 기술명", "reason": "추천 사유"}}, {{...}}, {{...}}]}}
"""
...
with col2:
    ...
    topic = st.text_input("확정된 기술명", value=st.session_state.get('step2_topic', ''), placeholder="기술명 입력 또는 Step 1에서 선택")
    ...
    if st.button("🚀 마스터 리포트 생성", type="primary", use_container_width=True):
        if not topic:
            ...
            with st.spinner("AI가 11개 섹션 작성 중..."):
                prompt = f"""당신은 중소기업 대표를 도와 벤처인증 신청서를 대신 써주는 사람입니다.

기술명: [{topic}]

위 기술에 대해 벤처인증 심사용 사업계획서 11개 항목을 작성하세요.

[★ 가장 중요한 규칙 - 글자수 ★]
- 각 항목(섹션)은 **공백 포함 1000자 이내**로 반드시 작성. 1000자를 절대 넘기지 마세요.
- 항목당 대략 600~950자 정도가 적당합니다. 넘치면 잘립니다.

[★ 문체 규칙 - 사람이 쓴 것처럼 ★]
- 이건 AI 보고서가 아니라 '회사 대표가 직접 쓴 신청서'입니다.
- 너무 매끄럽고 완벽한 마케팅 문구를 쓰지 마세요. 약간 투박하고 담백하게.
- 화려한 수식어("혁신적인", "압도적", "최첨단", "획기적") 남발 금지.
- 과장된 통계나 멋진 숫자를 지어내지 말 것. 모르면 일반적인 표현으로.
- 실무자가 쓰듯 약간 건조하고 담백한 문장. 가끔 평이한 표현도 섞어서.
- 완벽한 문장구조보다는 실제 사람이 쓴 듯 자연스럽게.

[형식 규칙]
1. 표(Table) 절대 금지 - '•' 또는 '1. 2. 3.' 텍스트
2. 항목별로 반드시 '### [번호. 항목명]' 형식
3. ★★★ 1번 항목은 아래 '표준 V양식'을 반드시 그대로 사용할 것 ★★★
   - V로 시작하는 문장의 구조(어순, 표현, 연결어)를 절대 바꾸지 말 것
   - 오직 빈칸(______)에 해당하는 내용만 채워넣을 것
   - 1번 항목도 전체 1000자 이내

### [1. 신청기술 요약 및 표준 양식]
- 신청기술(제품/서비스)명 : {topic}
- 신청기술(제품/서비스) 요약 : (기술명과 간략한 소개를 한 문장으로 작성)

아래 V양식의 빈칸(______)만 채워서 그대로 출력하세요. 문장 구조는 절대 변경 금지:
V 기존 시장에 ______니즈(문제)가 있는데, ______한 이유로 사람들이 여전히 필요로 하고(불편을 겪고) 있음
V 당사에서 ______한 방식으로 해결책을 찾았으며, 이는 기존시장의 기술(제품, 서비스)과 ______ 차이를 보유하고 있음
V 현재 당사에서 보유 또는 개발중인 기술(제품, 서비스)명은 ______로써, 전체 시장은 ______ 규모이며 이 기술(제품, 서비스)로 잠재고객들의 니즈를 충족시킬 경우 이 시장의 규모는 연평균 ___%의 성장을 기대할 수 있음
V 당사 기술(제품, 서비스)은 ______에 기반하여 ______한 특징을 갖고 있으며 ______한 이유로 기존 문제에 대한 혁신적인 해결책으로, 잠재 고객들의 만족도가 훨씬 높을 수 있음
V 기술(제품, 서비스)에 대한 ______ 등 ___건의 지식재산권과 ______ 및 ___명의 연구개발조직을 보유하여 ___건의 기술개발을 완료(수행) 하는 등 끊임없는 연구개발에 노력하고 있으며, 지속발전이 가능한 기술적 역량을 보유하고 있음
V 기술(제품, 서비스)을 통한 시장진입을 위해 ______ 등의 마케팅 활동을 진행중으로 현재 ______ 규모 정도의 시장을 확보하고 있으며, 시장확대를 위해 향후 3년간 ______ 등의 마케팅 계획을 수립하여 진행할 계획
V 당사가 제시하는 이러한 성과가 가능한 이유는 당사에 ______한 역량이 있기 때문이며 이를 더욱 강화하여 향후 ___년간 ______한 ______ 성장을 해낼 것임
...
### [10. 연계 가능 정책자금 추천]
### [11. 지식재산권 획득 컨설팅]
이 항목은 '이 기술로 어떤 지식재산권을 딸 수 있는지' 구체적으로 컨설팅하는 내용입니다.
- 벤처인증을 받은 뒤, 이 기술({topic})에서 실제로 출원 가능한 지식재산권을 콕 집어 제안할 것
- 막연한 일반론 금지. 이 기술의 어떤 요소가 어떤 권리가 되는지 구체적으로:
  · 특허 출원 가능 항목: 핵심 기술/알고리즘/공정/방법 중 무엇으로 몇 건 정도 가능한지
  · 실용신안 가능 항목: 구조나 형상 개선 중 무엇이 해당하는지
  · 디자인권 가능 항목: 제품 외관/UI 등 해당되면 제안
  · 상표권: 브랜드명/서비스명 보호 방안
- 각 권리별로 '왜 이게 출원 가능한지' 한 줄 근거를 붙일 것
- 출원 우선순위와 대략적 일정도 간단히 제안

[다시 강조] 각 항목은 반드시 1000자 이내. 사람이 직접 쓴 것처럼 담백하게.
"""
...
if "sections" in st.session_state:
    ...
    with dc1:
        st.download_button("💎 프리미엄 디자인 리포트 다운로드 (.html)",
                          data=html_content, file_name=f"RSV_벤처인증_리포트_{td}.html",
                          mime="text/html", type="primary", use_container_width=True)
    with dc2:
        st.caption("💡 HTML → 브라우저 열기 → Ctrl+P → PDF 저장!")
    st.divider()
    st.markdown("💡 **벤처인증 사이트에 붙여넣기:** 각 섹션 [복사] 버튼 → 항목별 붙여넣기")
